...
module.exports = function(temporaryFolder) {
  ...
  try {
    var stat = fs.statSync(path.join(__dirname, temporaryFolder))
    if (!stat) {
  ...
  function validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename, fileSize) {
    ...
    debugger
    var numberOfChunks = Math.max(Math.ceil(totalSize / chunkSize), 1)
  ...
  $.get = function(req, callback) {
    var chunkNumber = parseInt(req.query['chunkNumber'])
    var chunkSize = parseInt(req.query['chunkSize'])
    var totalSize = parseInt(req.query['totalSize'])
  ...
  $.post = function(req, callback) {
    ...
    var chunkNumber = parseInt(fields['chunkNumber'])
    var chunkSize = parseInt(fields['chunkSize'])
    var totalSize = parseInt(fields['totalSize'])
    ...
    if (validation === 'valid') {
      var chunkFilename = getChunkFilename(chunkNumber, identifier)
      const tmpFileExist = fs.existsSync(files[$.fileParameterName].path)
      if (tmpFileExist) {
        debugger
